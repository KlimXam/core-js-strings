...
function removeLastOccurrences(str, value) {
  const x = str.lastIndexOf(value);
  if (x !== -1) {
    return str.slice(0, x) + str.slice(x).replace(value, '');
  }
  return str;
}
...
function countVowels(str) {
  const vow = 'aAeEiIoOuUyY';
  let sum = 0;
  for (let x = 0; x < str.length; x += 1) {
    const y = str.charAt(x);
    if (vow.includes(y)) {
      sum += 1;
    }
  }
...
function findLongestWord(sentence) {
  const y = sentence.split(' ');
  let bigWord = y[0];
  for (let x = 1; x < y.length; x += 1) {
    const word = y[x];
...
function reverseWords(str) {
  const y = str.split(' ');
  const revWord = y.map((x) => x.split('').reverse().join(''));
  const revStr = revWord.join(' ');
  return revStr;
}
...
function invertCase(str) {
  ...
  for (let x = 0; x < str.length; x += 1) {
    const y = str[x];
    if (y === y.toUpperCase()) {
      invertStr += y.toLowerCase();
    } else {
      invertStr += y.toUpperCase();
    }
  }
  return invertStr;
}
